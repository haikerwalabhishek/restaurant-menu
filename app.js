...
function populate(category){
  ...
  for(item of menu){
    ...
    if(category===item.category || category==="all" || item.title.toLowerCase().includes(category) || item.category.toLowerCase().includes(category) || item.price.toLowerCase().includes(category) || item.description.toLowerCase().includes(category)){
      items.innerHTML+=`
      <div class="menu-item card-box">
        <div class="card">
          <img style="height: 20vh; width: 100%; border-start-start-radius: 10px; border-start-end-radius: 10px;" src="${item.img}" alt="${item.title}">
          <div class="card-body">
            <h5 class="card-title">${item.title}</h5><br>
            <h6 class="card-subtitle mb-2 text-muted"><b>${item.price}</b></h6><br>
            <p class="card-text">${item.description.length<=50?item.description:item.description.slice(0,70)+"..."}</p><hr>
  ...
  if(!itemFound){
    itemCont.innerHTML='<div class="items"></div> <h2>Sorry! No item found</h2>';
  };
} 
...
const searched = document.querySelector("input.form-control");

searched.addEventListener("input",()=>{
  let inputVal = searched.value;
  populate(inputVal.trim().toLowerCase());
